...
* Нет обработки ошибок, как следствие никаких гарантий at least once и тд, всё вываливается в логи( но продолжает работать и ничего не падает )
Так же из-за нехватки времени в приложении остаётся много захардкоженных штук, практически нет комментариев :C

## Ограничение хипа
Сейчас в докер контейнерах жёстко задаётся -Xmx512m.
Потенциально для Query Server может быть необходимость увеличить его в зависимости от нагрузки
